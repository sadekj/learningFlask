...
@app.route("/") ## thi creating a blank url, means if nothing in the url run the following funtion
...
def hello(name=None):## this is the function that will be called, notice it takes an input, and it should be the same name of the one in the url, and the default value is None, other wise the value of it will be what is written in the url
	...
	return render_template('hello.html', name=name)## here telling flask to load the html code in hello.html wht the variable name

## to create more urls and functions simple create them the same way created above just change what you need

if __name__ == "__main__": ## this if statement is telling python when runnig this file run the following lines
    app.debug = True ## this to let flask show us errors if ther is any
